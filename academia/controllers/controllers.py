...
class Academia(http.Controller):
    ...
    @http.route('/academia/professores/', auth='public')
    def professores(self, **kw):
        Professores = http.request.env['academia.professores']
        return http.request.render('academia.professores', {
            'professores': Professores.search([])
            })
